...
const EMPTY_FORM = (defaultType) => ({
  name: '', email: '', phone: '', message: '', quantity: '', inquiryType: defaultType,
});

export default function ProductInquiryDialog({ open, onClose, productId, defaultType = 'GENERAL' }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await shopApi.createInquiry(payload);
  ...
  const types = productId ? ['QUOTE', 'STOCK_CHECK', 'GENERAL'] : ['GENERAL'];
  const showQuantity = form.inquiryType === 'QUOTE';
      ...
      <form onSubmit={handleSubmit} noValidate>
        ...
        <DialogContent>
          <Stack spacing={2} sx={{ mt: 1 }}>
            ...
            {showQuantity && (
              <TextField
                type="number" label={t('inquiry.quantity')} value={form.quantity}
                onChange={update('quantity')} fullWidth inputProps={{ min: 1 }}
              />
            )}
