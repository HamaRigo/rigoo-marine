...
// Static sx objects defined once at module scope — never recreated on render.
const cardSx = {
  height: '100%',
  display: 'flex',
...
const cardOutSx = { ...cardSx, opacity: 0.85 };
const actionAreaSx = { display: 'flex', flexDirection: 'column', alignItems: 'stretch', height: '100%' };
const placeholderSx = {
  height: 200, bgcolor: 'grey.100', display: 'flex',
...
function ProductCard({ product, index = 0 }) {
  ...
  const stockChip = isOut
    ...
    : isLow
    ? { label: t('card.lowStock', { count: stock }), color: 'warning' }
    : { label: t('card.inStock'), color: 'success' };

  const chipPositionKey = isAr ? 'left' : 'right';
  const catChipPositionKey = isAr ? 'right' : 'left';

  return (
    <Grow in timeout={400} style={{ transitionDelay: `${Math.min(index, 12) * 60}ms` }}>
      <Card sx={isOut ? cardOutSx : cardSx}>
        <CardActionArea
        ...
        >
          <Box sx={{ position: 'relative' }}>
            ...
            <Chip
              ...
              size="small"
              sx={{ position: 'absolute', top: 12, [chipPositionKey]: 12, fontWeight: 600 }}
            />
            {product.category && (
              <Chip
                label={t(`categories.${product.category}`, product.category)}
                size="small"
                sx={{ position: 'absolute', bottom: 12, [catChipPositionKey]: 12, bgcolor: 'rgba(0,0,0,0.6)', color: 'white' }}
              />
            )}
          </Box>

          <Box sx={{ p: 2, flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
            <Typography variant="h6" sx={{ mb: 0.5, lineHeight: 1.3 }} noWrap>{name}</Typography>
