...
( cd rigoo-marine-backend && mvn -q -DskipTests \
    -pl discovery-service,gateway-module,client-module,vessel-module,service-module,work-order-module,technician-module,invoice-module,notification-module \
    -am package )
...
while :; do
    ...
    if [ "$healthy" -ge 11 ] && [ "$starting" -eq 0 ]; then break; fi
